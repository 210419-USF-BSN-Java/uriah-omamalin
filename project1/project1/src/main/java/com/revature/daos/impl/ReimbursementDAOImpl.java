...
public class ReimbursementDAOImpl implements ReimbursementDAO {
	...
	public void create(Reimbursement t) {
		String sql = "insert into ers.reimbursement (reimb_amount, reimb_submitted, reimb_description, reimb_author, reimb_status_id, reimb_type_id) "
				   + "values (?, ?, ?, ?, 1, ?)";
		...
		try (Connection con = ConnectionUtil.getConnectionFromFile()) {
			...
			ps.setInt(4, t.getReimbAuthor());
			ps.setInt(5, t.getReimbTypeId());
	...
	@Override
	public List<Reimbursement> getResolvedReimbursementsByAuthorId(int authorId) {
		List<Reimbursement> li = new ArrayList<Reimbursement>();
		String sql = "select * from ers.reimbursement where reimb_status_id != 1 and reimb_author = ?";
		try (Connection con = ConnectionUtil.getConnectionFromFile()) {
			PreparedStatement ps = con.prepareStatement(sql);
	...
	public List<Reimbursement> getAllReimbursementsByAuthorId(int authorId) {
		List<Reimbursement> li = new ArrayList<Reimbursement>();
		String sql = "select * from ers.reimbursement where reimb_author = ?";
		try (Connection con = ConnectionUtil.getConnectionFromFile()) {
			PreparedStatement ps = con.prepareStatement(sql);
			ps.setInt(1, authorId);
			ResultSet rs = ps.executeQuery();
